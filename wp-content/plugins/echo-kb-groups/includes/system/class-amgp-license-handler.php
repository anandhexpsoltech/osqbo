...
class AMGP_License_Handler {
	...
	public function get_cached_info() {
		$default_array = array( 'Unknown status.', '', Echo_KB_Groups::$version, '' );
		$amgp_license_state = AMGP_Utilities::get_wp_option( self::AMGP_LICENSE_STATE, $default_array, true );
		$status_msg = isset($amgp_license_state[0]) ? $amgp_license_state[0] : 'Unknown status. ';
	...
	private function save_license( $license_key ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( empty($message) ) {
				AMGP_Utilities::ajax_show_error_die( $result->get_error_message(), 'Could not save settings.' );
			}
			AMGP_Utilities::ajax_show_error_die( $this->generate_error_summary( $result->get_error_data() ), 'Settings NOT saved due to following problems:' );
		}
	}
		...
		switch( $error_code ) {
			...
			case 'no_activations_left':  // 'activate' action
				...
				$message = 'This license key is registered on other ' . $max_sites . ' website' . ($max_sites == 1 ? '' : 's') . ' which is the license limit. ' .
				           ...
				           'a) increase the number of websites the license can handle: <a href="https://www.echoknowledgebase.com/wordpress-plugin/kb-groups//">View possible upgrades</a> OR ' .

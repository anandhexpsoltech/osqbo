...
final class Echo_KB_Groups {
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( in_array($action, array( 'amgp_add_kb_groups_tabs_ajax', 'amgp_add_kb_group_ajax', 'amgp_delete_kb_group_ajax', 'amgp_rename_kb_group_ajax' ) ) ) {
...
function amgp_display_version_error() {
	...
	if ( $plugins_check == 'core_missing' ) {
		echo '<div class="error"><p>KB Groups plugin requires <a href="https://wordpress.org/plugins/echo-knowledge-base/"
	...
	if ( $plugins_check == 'core_too_old' ) {
		echo '<div class="error">KB Groups plugin requires at least ' . Echo_KB_Groups::$needs_min_core_version .
		...
		echo '<div class="error">' . $kb_plugin_name . ' plugin requires the most recent version of the KB Groups plugin. The old version of KB Groups stays
			<strong>disabled</strong> until updated. <a href="' . $add_on_url .'" target="_blank">Check its status here.</a></p></div>';
	} else {
	    // should not happen
	    echo '<div class="error">KB Groups: unknown add-on version conflict.</div>';
    }
}

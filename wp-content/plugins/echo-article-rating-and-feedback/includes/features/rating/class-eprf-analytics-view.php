...
class EPRF_Analytics_View {
    ...
    public function __construct() {
	    ...
	    // TODO add_action( 'wp_ajax_nopriv_eprf_handle_rating_analytics', array( $this, 'user_not_logged_in' ) );
    }

	/**
	 * Add RATING DATA button to end of Top navigation
	 */
	public function display_navigation_button() {           ?>
		<!--  RATING PAGE BUTTON -->
		<div class="eckb-nav-section">
			<div class="page-icon-container">
				<p><?php _e( 'Rating Data', 'eprf-knowledge-base' ); ?></p>
				<div class="page-icon epkbfa epkbfa-thumbs-up" id="eprf-rating-data"></div>
			</div>
		</div>	<?php
	}
	...
	public function add_analytics_content( $kb_id ) {
		...
		foreach( $least_rated_articles_list as $most_rated_article ) {
			...
			$post = EPRF_Utilities::get_kb_post_secure( $most_rated_article->post_id, true );
		...
		foreach( $least_rated_articles_list as $least_rated_article ) {
			...
			$post = EPRF_Utilities::get_kb_post_secure( $least_rated_article->post_id, true );
		...
		<div class="eckb-config-content" id="eprf-rating-data-content">
			...
			<!-- ROW 3 --------------------------------------------------->
			<!-- <div class="eckb-row eprf-rating-tables"> -->
	...
	public function user_not_logged_in() {
		EPRF_Utilities::ajax_show_error_die( '<p>' . __( 'You are not logged in. Refresh your page and log in', 'echo-article-rating-and-feedback' ) . '.</p>', __( 'Cannot save your changes', 'echo-article-rating-and-feedback' ) );
	}

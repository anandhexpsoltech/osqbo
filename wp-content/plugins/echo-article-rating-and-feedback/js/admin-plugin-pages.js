jQuery(document).ready(function($) {
	...
	function showStatistics(stars) {
		...
		if( $( '.eprf-ratings-gutenberg-editor-container' ).length > 0 ){
			...
			$( 'body .eprf-ratings-gutenberg-editor-container' ).find( '.eprf-stars-module__statistics' ).css(
				{
					"position" : "fixed",
	...
	$(document.body).on('click', '#resetArticleRating', function(){
		...
		if ( typeof eprf_article_rating_data == 'undefined' ) {
			// should never run, added just in case
			alert( 'Missing EPRF rating data.' );
			return true;
		}
		
		if (confirm(eprf_article_rating_data.delete_confirm)) {
			// find article id
			...
			let postData = {
				...
				_wpnonce_edit_article: eprf_article_rating_data._wpnonce_edit_article,
				article_id: articleId
			};

			send_request( 'POST', postData, 'Wait please...' );

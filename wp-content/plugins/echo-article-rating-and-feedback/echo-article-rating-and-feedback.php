...
final class Echo_Article_Rating_And_Feedback {
	...
	public static $needs_min_core_version = '4.8.0';
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( in_array($action, array(EPRF_KB_Core::EPRF_SAVE_KB_CONFIG_CHANGES, EPRF_KB_Core::EPRF_SAVE_KB_ADVANCED_CONFIG_CHANGES) )  ) {
			new EPRF_KB_Config_Controller();
			return;
		}
	...
	private function setup_backend_classes() {
		...
		if ( $pagenow == 'post.php' ) {
			...
			$post = EPRF_Utilities::get_kb_post_secure( $post_id, false );
...
function eprf_display_version_error() {
	...
	if ( $plugins_check == 'core_missing' ) {
		echo '<div class="error"><p>KB - Article Rating and Feedback requires <a href="https://wordpress.org/plugins/echo-knowledge-base/"
	...
	if ( $plugins_check == 'core_too_old' ) {
		echo '<div class="error">Article Rating and Feedback plugin requires at least ' . Echo_Article_Rating_And_Feedback::$needs_min_core_version .
	    ...
	    echo '<div class="error">' . $kb_plugin_name . ' plugin requires the most recent version of the Article Rating and Feedback plugin. The old version of Article Rating and Feedback stays
			<strong>disabled</strong> until updated. <a href="' . $add_on_url .'" target="_blank">Check its status here.</a></p></div>';
	} else {
	    // should not happen
	    echo '<div class="error">Article Rating and Feedback: unknown add-on version conflict.</div>';
    }
}

...
final class Echo_Elegant_Layouts {
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( in_array($action, array(ELAY_KB_Core::ELAY_SAVE_KB_CONFIG_CHANGES, ELAY_KB_Core::ELAY_SAVE_KB_ADVANCED_CONFIG_CHANGES) )  ) {
			new ELAY_KB_Config_Controller();
			return;
		}
	...
	private function setup_backend_classes() {
		...
		if ( $pagenow == 'post.php' ) {
			...
			$post = ELAY_Utilities::get_kb_post_secure( $post_id, false );
		...
		if ( $request_page == ELAY_KB_Core::ELAY_KB_CONFIGURATION_PAGE || $pagenow == 'post.php' || $pagenow == 'post-new.php' ) {
...
function elay_display_version_error() {
	...
	if ( $plugins_check == 'core_missing' ) {
		echo '<div class="error"><p>KB - Elegant Layouts requires <a href="https://wordpress.org/plugins/echo-knowledge-base/"
	...
	if ( $plugins_check == 'core_too_old' ) {
		echo '<div class="error">Elegant Layouts plugin requires at least ' . Echo_Elegant_Layouts::$needs_min_core_version .
	    ...
	    echo '<div class="error">' . $kb_plugin_name . ' plugin requires the most recent version of the Elegant Layouts plugin. The old version of Elegant Layouts stays
			<strong>disabled</strong> until updated. <a href="' . $add_on_url .'" target="_blank">Check its status here.</a></p></div>';
	} else {
	    // should not happen
	    echo '<div class="error">Elegant Layouts: unknown add-on version conflict.</div>';
    }
}

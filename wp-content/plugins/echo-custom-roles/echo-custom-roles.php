...
final class Echo_Custom_roles {
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( in_array($action, array( 'amcr_display_wp_roles_tabs_ajax', 'amcr_add_wp_role_ajax', 'amcr_delete_wp_role_ajax' ) ) ) {
...
function amcr_display_version_error() {
	...
	if ( $plugins_check == 'core_missing' ) {
		echo '<div class="error"><p>Custom Roles requires <a href="https://wordpress.org/plugins/echo-knowledge-base/"
	...
	if ( $plugins_check == 'core_too_old' ) {
		echo '<div class="error">Custom Roles plugin requires at least ' . Echo_Custom_roles::$needs_min_core_version .
		...
		echo '<div class="error">' . $kb_plugin_name . ' plugin requires the most recent version of the Custom Roles plugin. The old version of Custom Roles stays
			<strong>disabled</strong> until updated. <a href="' . $add_on_url .'" target="_blank">Check its status here.</a></p></div>';
	} else {
	    // should not happen
	    echo '<div class="error">Custom Roles: unknown add-on version conflict.</div>';
    }
}

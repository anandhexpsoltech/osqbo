...
class AMCR_License_Handler {
	...
	public function get_cached_info() {
		$default_array = array( 'Unknown status.', '', Echo_Custom_roles::$version, '' );
		$amcr_license_state = AMCR_Utilities::get_wp_option( self::AMCR_LICENSE_STATE, $default_array, true );
		$status_msg = isset($amcr_license_state[0]) ? $amcr_license_state[0] : 'Unknown status. ';
	...
	public function handle_license_command() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_amcr_license_key'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce_amcr_license_key'], '_wpnonce_amcr_license_key' ) ) {
			AMCR_Utilities::ajax_show_error_die('Security check failed.');
		}
	...
	private function save_license( $license_key ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( empty($message) ) {
				AMCR_Utilities::ajax_show_error_die( $result->get_error_message(), 'Could not save settings.' );
			}
			AMCR_Utilities::ajax_show_error_die( $this->generate_error_summary( $result->get_error_data() ), 'Settings NOT saved due to following problems:' );
		}
	}
		...
		switch( $error_code ) {
			...
			case 'no_activations_left':  // 'activate' action
				...
				$message = 'This license key is registered on other ' . $max_sites . ' website' . ($max_sites == 1 ? '' : 's') . ' which is the license limit. ' .
				           ...
				           'a) increase the number of websites the license can handle: <a href="https://www.echoknowledgebase.com/wordpress-plugin/custom-roles/">View possible upgrades</a> OR ' .

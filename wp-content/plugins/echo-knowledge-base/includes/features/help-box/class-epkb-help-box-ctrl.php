...
/**
 * Handle user submission from Help Box
 */
class EPKB_Help_Box_Ctrl {

	public function __construct() {

		add_action( 'wp_ajax_epkb_help_box_contact', array($this, 'submit_contact_form' ) );
		add_action( 'wp_ajax_nopriv_epkb_help_box_contact', array($this, 'submit_contact_form' ) );
	}
	...
	public function submit_contact_form() {
		...
		if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], '_epkb_help_box_contact_form_nonce' ) ) {
		...
		// send the email
		$result = EPKB_Utilities::send_email( $message, $epkb_settings['help_box_contact_submission_email'], $reply_to_email, $reply_to_name, $subject, __( 'New KB Help Request', 'echo-knowledge-base' ) );
		if ( empty( $result ) ) {
			wp_send_json_success( $epkb_settings['help_box_contact_success_message'] );
		} else {

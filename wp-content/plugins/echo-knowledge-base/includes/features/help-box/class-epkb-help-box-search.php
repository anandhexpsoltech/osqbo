...
class EPKB_Help_Box_Search {

	public function __construct() {

		add_action( 'wp_ajax_epkb_help_box_get_category_list', array($this, 'get_category_list') );
		add_action( 'wp_ajax_nopriv_epkb_help_box_get_category_list', array($this, 'get_category_list') );

		add_action( 'wp_ajax_epkb_help_box_article_detail', array($this, 'get_article_detail' ) );
		add_action( 'wp_ajax_nopriv_epkb_help_box_article_detail', array($this, 'get_article_detail' ) );

		add_action( 'wp_ajax_epkb_help_box_search_kb', array($this, 'search_kb') );
		add_action( 'wp_ajax_nopriv_epkb_help_box_search_kb', array($this, 'search_kb') );
	}
	...
	public function get_category_list() {
		...
		$kb_category_ids = empty($settings['help_box_faqs_category_ids']) ? [] : explode(',', $settings['help_box_faqs_category_ids']);
		...
		$category_list = "<ul class='epkb-help_box_categories'>";
		foreach( $kb_category_ids as $kb_category_id ) {
			...
			$category_list .= '<li id="epkb-help_box_cat-' . $kb_category->term_id . '" class="epkb-help_box_cat-item" data-category="' . $kb_category->term_id. '">
					<span class="epkb-help_box_cat-name">
						<span class="epkb-help_box_cat-name__icon epkbfa epkbfa-folder-o"></span>
						<span class="epkb-help_box_cat-name__text">' . $kb_category->name . '</span>
					</span>
			</li>';
		}
		$category_list .= "</ul>";
		...
		foreach( $displayed_category_ids as $kb_category_id ) {
			$article_list .= '<div class="epkb-help_box_article-box" data-category="' . $kb_category_id . '">';
	...
	public function get_article_detail() {
		...
		$read_more = epkb_get_instance()->settings_obj->get_value( 'help_box_faqs_read_more_text' );
		$search_result =
			"<div class='epkb-help_box_article-item-details epkb-help_box_type_" . $data_type . "'>
				<div class='epkb-help_box_article-title'>" . get_the_title( $article_id ) . "</div>
				<div class='epkb-help_box_article-desc'>" . get_the_excerpt( $article_id ) . "</div>
				<a class='epkb-help_box_article-link' href='" . get_permalink( $article_id ) . "' target='_blank'>". $read_more . "</a>
			</div>";
	...
	/**
	 * Search Help Box KB for articles within defined categories
	 */
	public function search_kb() {
		...
		foreach ( $result as $post ) {
			...
			$search_result .=
				'<li data-type="search" class="epkb-help_box_article-item" ' . $article_data . ' >  
											
					<span class="epkb-help_box_article-item__name">
						<span class="epkb-help_box_article-item__icon ' . $article_title_icon . '"></span>
						<span class="epkb-help_box_article-item__text">' . esc_html($post->post_title) . '</span>
					</span>
	...
	private static function article_list_by_category( $kb_id, $kb_category_id ) {
		...
		if ( isset($articles_seq_data[ $kb_category_id]) ) {
			...
			$search_result .= '<h3 class="epkb-help_box__category_title">' . $articles_list[0] . '</h3>';
		...
		$search_result .= '<ul class="epkb-help_box__article_list">';
		foreach ( $articles_list as $article_id => $article_title ) {

			$article_data = 'data-kb-article-id=' . $article_id . ' data-kb-id=' . $kb_id;

			$search_result .=
				'<li data-type="category" class="epkb-help_box_article-item" ' . $article_data . ' >  
					<span class="epkb-help_box_article-item__name">
						<span class="epkb-help_box_article-item__icon ep_font_icon_document"></span>
						<span class="epkb-help_box_article-item__text">' . $article_title . '</span>
					</span>
			   	</li>';
		}
		$search_result .= '</ul>';

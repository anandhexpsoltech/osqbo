...
final class Echo_Knowledge_Base {
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( $action == 'epkb_help_box_get_category_list' || $action == 'epkb_help_box_article_detail' || $action == 'epkb_help_box_search_kb' ) {
			new EPKB_Help_Box_Search();
			return;
		}

		if ( $action == 'epkb_help_box_contact' ) {
			new EPKB_Help_Box_Ctrl();
			return;
		}

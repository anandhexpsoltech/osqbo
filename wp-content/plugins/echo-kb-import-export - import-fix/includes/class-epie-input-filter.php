...
class EPIE_Input_Filter {
	...
	public function validate_and_sanitize_specs( array $input, array $specification ) {
		...
		foreach ($input as $key => $input_value) {
			...
			if ( is_wp_error($result) ) {
                ...
                EPIE_Logging::add_log( 'Please change the value of ' . $field_spec['label'] . ' field. Current value: "' . $input_value . '" - ' . $result->get_error_message() . ', code: ' . $result->get_error_message(), $result );
	...
	private function filter_number( $number, $field_spec=array() ) {
		...
		$number_int = EPIE_Utilities::sanitize_int( $number, null );
	...
	private function filter_id( $id ) {
		$id = EPIE_Utilities::sanitize_get_id( $id );
		if ( is_wp_error($id) ) {

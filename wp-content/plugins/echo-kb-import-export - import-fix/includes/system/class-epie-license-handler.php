...
class EPIE_License_Handler {
	...
	const PLUGIN_NAME           = 'KB Import Export';
	const LICENSE_KEY_OPTION_ID = 'epie_license_key';
	const EPIE_LICENSE_STATE    = 'epie_license_state';
	...
	function __construct( $_file, $_version ) {
		...
		$this->license_key = EPIE_Utilities::get_wp_option( self::LICENSE_KEY_OPTION_ID, '' );
		$this->setup_hooks();
		$this->auto_updater();
	}
	...
	private function setup_hooks() {
		add_filter( EPIE_KB_Core::EPIE_KB_ADD_ON_LICENSE_MSG, array( $this, 'get_license_status_short') );
	}
	...
	private function auto_updater() {
		...
		// setup the updater
		if ( ! class_exists( 'EPIE_EDD_SL_Plugin_Updater' ) ) {
			require_once 'class-epie-edd-sl-plugin-updater.php';
		}

		new EPIE_EDD_SL_Plugin_Updater(
	...
	public function get_license_status_short( $output ) {
		...
		$licenses_tab_url = admin_url('edit.php?post_type=' . EPIE_KB_Handler::get_post_type( EPIE_KB_Core::DEFAULT_KB_ID ) . '&page=ep'.'kb-add-ons&ep'.'kb-tab=licenses');
		$output_status = array();

		// 1. ADD-ON VERSION
		if ( ! empty($stored_license_data['new_version']) ) {
			$output_status[] = sprintf( __( 'Please update %s to its latest version %s' , 'echo-knowledge-base' ), '<strong>' . self::PLUGIN_NAME . '</strong>', $stored_license_data['new_version'] ) . '<a href="' . $licenses_tab_url . '" target="_blank">' . __( 'Click here for details.', 'echo-knowledge-base' ) . '</a>';
		}
	...
	public function get_cached_info() {
		$default_array = array( __( 'Unknown status.', 'echo-knowledge-base' ), '', Echo_KB_Import_Export::$version, '' );
		$epie_license_state = EPIE_Utilities::get_wp_option( self::EPIE_LICENSE_STATE, $default_array, true );
		$status_msg = isset($epie_license_state[0]) ? $epie_license_state[0] : __( 'Unknown status.', 'echo-knowledge-base' );
		$license_state = isset($epie_license_state[1]) ? $epie_license_state[1] : '';
		$latest_add_on_version = isset($epie_license_state[2]) ? $epie_license_state[2] : Echo_KB_Import_Export::$version;
		$expiry_date = isset($epie_license_state[3]) ? $epie_license_state[3] : '';
	...
	public function handle_license_command() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_epie_license_key'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce_epie_license_key'], '_wpnonce_epie_license_key' ) ) {
			EPIE_Utilities::ajax_show_error_die( __( 'Security check failed.', 'echo-knowledge-base' ) );
		}

		// need license key to activate/deactivate the license
		$entered_license_key = empty($_REQUEST['epie_license_key']) ? '' : sanitize_text_field( $_REQUEST['epie_license_key'] );
	...
	private function save_license( $license_key ) {
		...
		$result = EPIE_Utilities::save_wp_option( self::LICENSE_KEY_OPTION_ID, $license_key, true );
		if ( is_wp_error( $result ) ) {
			/* @var $result WP_Error */
			$message = $result->get_error_data();
			if ( empty($message) ) {
				EPIE_Utilities::ajax_show_error_die( $result->get_error_message(),  __( 'Could not save settings.', 'echo-knowledge-base' ) );
			}
			EPIE_Utilities::ajax_show_error_die( $this->generate_error_summary( $result->get_error_data() ), __( 'Settings NOT saved due to following problems:', 'echo-knowledge-base' ) );
		}
	}
	...
	public function contact_license_server( $action, $license_key ) {
		...
		if ( is_wp_error( $response ) ) {
			$error_msg = '<br/>REQUEST: url => ' . $get_url . ', args => ' . EPIE_Utilities::get_variable_string( $get_args );
			$error_msg .= '<br/>RESPONSE: ' . EPIE_Utilities::get_variable_string( $response );
	...
	public function retrieve_status_message( $license_data, $license_key ) {
		...
		switch( $error_code ) {
			...
			case 'no_activations_left':  // 'activate' action
				...
				$message = sprintf( __( 'This license key is registered on other %s website%s which is the license limit. In order to register the license on this website you can either: a) increase the number of websites the license can handle: %s View possible upgrades %s OR b) de-register your old website URL and register this website URL. Please %s to make the URL change.', 'echo-knowledge-base' ), $max_sites, ($max_sites == 1 ? '' : 's'), '<a href="https://www.echoknowledgebase.com/wordpress-plugin/advanced-search/">', '</a>', $visit_account_page );
				break;
	...
	public function get_expiry_warning( $expiry_date ) {
		...
		$difference_in_days = EPIE_Utilities::get_days_since( '', $expiry_date );
		...
		if ( $difference_in_days > 0 ) {
			return sprintf( __( 'Your license will expire in %s days.', 'echo-knowledge-base' ), $difference_in_days );
		} else {
			return __( 'Your license ', 'echo-knowledge-base' ) . ( $difference_in_days < 0 ? __( 'expired ', 'echo-knowledge-base' ) . abs($difference_in_days) . __( ' days ago. ', 'echo-knowledge-base' ) : __( ' will expire today. ', 'echo-knowledge-base' ) . $msg ) ;
		}
	}
	...
	public function get_license_state( $force_refresh=false ) {
		...
		$stored_license_data = EPIE_Utilities::get_wp_option('epie_last_license_check', array(), true, true);
		if ( is_wp_error($stored_license_data) ) {
			EPIE_Logging::add_log("Could not retrieve license data option.");
			return null;
		}
		...
		if ( ! empty($license_error) && $this->is_connection_error( $license_error ) ) {
			...
			if ( ! empty($stored_license_data['last_connection_error']) && EPIE_Utilities::get_days_since( $stored_license_data['last_connection_error'] ) > 30 ) {
		...
		if ( ! empty($add_on_data) &&  is_object( $add_on_data ) && ! empty( $add_on_data->new_version ) ) {
			...
			EPIE_Logging::add_log("Could not contact licensing server.", ( isset($add_on_data->license) ? $add_on_data->license : '' ) );
			$cached_info = $this->get_cached_info();
			$latest_add_on_version = $cached_info['latest_add_on_version'];
		}
		$stored_license_data['new_version'] = version_compare( Echo_KB_Import_Export::$version, $latest_add_on_version, "<" ) == 0 ? null : $latest_add_on_version;
		...
		if ( ! empty($license_error) && ! in_array($license_error, array('site_inactive', 'expired', 'no_activations_left', 'disabled', 'license_not_activable')) ) {
			...
			$expiry_msg = epie_get_instance()->license_handler->get_expiry_warning( $expiry_date );
		}
		...
		if ( epie_get_instance()->is_dormant ) {
		...
		EPIE_Utilities::save_wp_option('epie_last_license_check', $stored_license_data, true);

		// store last known status
		EPIE_Utilities::save_wp_option( EPIE_License_Handler::EPIE_LICENSE_STATE, array( $status_msg, $license_state, $latest_add_on_version, $expiry_date ), true );

...
class EPIE_KB_Import_Processor {
	...
	public function import_kb_content() {
		...
		if ( $import_action == 'import_data' ) {
			...
			$result = set_transient( self::IMPORT_ARTICLES_TO_IMPORT, $articles_to_import, HOUR_IN_SECONDS );
			if ( empty($result) ) {
				wp_die( json_encode( array( 'error' => __( 'Could not import articles. Invalid content.', 'echo-kb-import-export' ) ) ) );
			}
	...
	private function parse_csv_data( $fp ) {
        ...
        while ( ($row = fgetcsv($fp)) !== FALSE ) {
            ...
            foreach ( $header as $row_ix => $column_name ) {
                ...
                //Special Charactor Encoding
				$encoding = mb_detect_encoding($row[$row_ix], mb_detect_order(), true);
				$row[$row_ix] = iconv($encoding, "UTF-8", $row[$row_ix]);
    ...
    private function prepare_kb_articles_for_import( $kb_id, $articles_to_import ) {
	    ...
	    if ( $count_overwrite > 0 ) {
		    $response_html .= $this->output_table( 'Overwrite Articles',  __( 'Lists all rows that represent existing articles. The articles title, content and categories will be overwritten and the article status set to Draft.',  'echo-kb-import-export' ), $table_header, $table_rows, 'override' );
	    }
	    ...
	    if ( $count_overwrite > 0 ) {
		    $response_html .= $this->output_table( 'New Articles', __( 'Lists articles that will be created (as a Draft) with given categories assigned.', 'echo-kb-import-export' ), $table_header, $table_rows, 'new' );
	    }

	...
	( function () {
		...
		if($( 'body' ).is('[class*="post-type-ep' + 'kb_post_type"]')) {
			...
			if( $( '.page-title-action' ).length > 0 ) {
				...
				var add_new_article_url = $( '.page-title-action' ).attr('href');

				$( '.page-title-action' ).after( '<a href="'+add_new_article_url+'&linked-editor=yes" class="kblk-page-title-action">Add New LINK Article</a>')

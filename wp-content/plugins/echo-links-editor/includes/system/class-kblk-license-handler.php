...
class KBLK_License_Handler {
	...
	const LICENSE_SERVER_URL    = 'https://www.echoknowledgebase.com/';  	// site with registered EDD plugin to handle license
	...
	public function get_license_status_short( $output ) {
		...
		if ( empty($stored_license_data) ) {
			$output[self::PLUGIN_NAME] = 'Could not retrieve license state';
			return $output;
		}
		...
		if ( ! empty($stored_license_data['new_version']) ) {
			$output_status[] = 'Please update <strong>' . self::PLUGIN_NAME . '</strong> to its latest version ' . $stored_license_data['new_version'] . '. ' .
			                                   '<a href="' . $licenses_tab_url . '" target="_blank">Click here for details.</a>';
		}
		...
		$output[self::PLUGIN_NAME] = $list . '</ul>' . '<br/>Check current license status <a href="' . $licenses_tab_url . '" target="_blank">here.</a>';

		return $output;
	}

	public function get_cached_info() {
		$default_array = array( 'Unknown status.', '', Echo_Links_Editor::$version, '' );
		$kblk_license_state = KBLK_Utilities::get_wp_option( self::KBLK_LICENSE_STATE, $default_array, true );
		$status_msg = isset($kblk_license_state[0]) ? $kblk_license_state[0] : 'Unknown status. ';
	...
	public function handle_license_command() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_kblk_license_key'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce_kblk_license_key'], '_wpnonce_kblk_license_key' ) ) {
			KBLK_Utilities::ajax_show_error_die('Security check failed.');
		}
	...
	private function save_license( $license_key ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( empty($message) ) {
				KBLK_Utilities::ajax_show_error_die( $result->get_error_message(), 'Could not save settings.' );
			}
			KBLK_Utilities::ajax_show_error_die( $this->generate_error_summary( $result->get_error_data() ), 'Settings NOT saved due to following problems:' );
		}
	}
	...
	public function contact_license_server( $action, $license_key ) {
		...
		if ( empty($license_key) && $action != 'get_version' ) {
			return $this->set_error_object( 'empty_license', 'Please enter your license.' );
		}
		...
		// Call the API
		$get_url = trailingslashit( self::LICENSE_SERVER_URL );
	...
	public function retrieve_status_message( $license_data, $license_key ) {
		...
		$visit_account_page = '<a href="https://www.echoknowledgebase.com/your-account" target="_blank" title="visit your account page">visit your account page</a>';

		$error_code = empty($license_data->error) ? '[unknown error]' : trim($license_data->error);
		switch( $error_code ) {

			case 'empty_license':
				$message = 'License for <strong>' . self::PLUGIN_NAME . '</strong> is missing. Please enter and activate your license.';
				break;
			...
			case 'expired' : // 'check_license' and 'activate' action
				...
				$message = sprintf('Your license key expired on %s. Please <a href="%s" target="_blank" title="Renew your license key">renew your license key</a>.',
									$expired_date, 'https://www.echoknowledgebase.com/checkout/?edd_license_key=' . $license_key);
				break;

			case 'missing' :
				$message = 'Invalid license key. Please ' . $visit_account_page . ' and verify it.';
				break;

			case 'site_inactive' :  // 'check_license' action
				$message = 'This license is not active for this URL. Either activate (save) the license or ' . $visit_account_page . ' to manage your license key URLs.';
				break;

			case 'invalid' :        // 'check_license' action
				$message = 'The entered license is not valid. Please enter a valid license or ' . $visit_account_page . ' to manage your license key(s).';
				break;

			case 'item_name_mismatch':  // 'check_license' and 'activate' action
			case 'invalid_item_id':	    // 'check_license' and 'activate' action
				$message = 'This license is not for ' . self::PLUGIN_NAME;
				break;

			case 'no_activations_left':  // 'activate' action
				$max_sites = isset( $license_data->max_sites) ?  $license_data->max_sites : 0;
				$message = 'This license key is registered on other ' . $max_sites . ' website' . ($max_sites == 1 ? '' : 's') . ' which is the license limit. ' .
				           'In order to register the license on this website you can either: ' .
				           'a) increase the number of websites the license can handle: <a href="https://www.echoknowledgebase.com/wordpress-plugin/links-editor-for-pdfs-and-more/">View possible upgrades</a> OR ' .
						   'b) de-register your old website URL and register this website URL. Please ' . $visit_account_page . ' to make the URL change.';
				break;
			...
			case 'failed_to_deactivate':
				$message = 'Error occurred (a30)' . ' - ' . $error_code;
				break;

			case 'key_mismatch':    // 'activate' action see EDD_Software_Licensing
				$message = 'Error occurred (a31)' . ' - ' . $error_code;
				break;

			default:
				$message = 'Error occurred (a32)' . ' - ' . $error_code;
				break;
		}
	...
	public function get_expiry_warning( $expiry_date ) {
		...
		$msg = 'Please <a href="https://www.echoknowledgebase.com/checkout/?edd_license_key=' . esc_attr($this->license_key) .
		        '" target="_blank" title="Renew your license key">renew your license key</a>.';

		if ( $difference_in_days > 0 ) {
			return 'Your license will expire in ' . $difference_in_days . ' days. ';
		} else {
			return 'Your license ' . ( $difference_in_days < 0 ? 'expired ' . abs($difference_in_days) . ' days ago. ' : ' will expire today. ' . $msg ) ;
		}
	}
	...
	public function get_license_state( $force_refresh=false ) {
		...
		if ( ! empty($license_error) && $this->is_connection_error( $license_error ) ) {
			...
			if ( ! empty($stored_license_data['last_connection_error']) && KBLK_Utilities::get_days_since( $stored_license_data['last_connection_error'] ) > 30 ) {
				$stored_license_data['error'] = 'Could not contact licensing server.<a href="' . $licenses_tab_url . '" target="_blank">Check your license status here.</a>';
				return $stored_license_data;
			}
		...
		if ( kblk_get_instance()->is_dormant ) {
			$stored_license_data['warning'] = 'The add-on will not run until both this add-on and KB core have compatible versions. See the message at the top of the page for details.';
		} else if ( $license_state != 'valid' ) {
			$stored_license_data['warning'] = 'Your license has to be both <strong>active</strong> and <strong>valid</strong> in order to receive <strong>add-on fixes, new features and support</strong>.<br/>';
		}

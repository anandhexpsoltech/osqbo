...
jQuery(document).ready(function($) {
	...
	function epie_update_export_progress_view() {
		...
		if ( epie_export_data.info.in_progress ) {
			
			$('#epie-content-export .epie-progress__bar').show();
			
			if ( Object.keys(epie_export_data.data).length ) {
				for ( let key in epie_export_data.data ) {
					
					/*
						epie_export_data.data[key] = {
							'ids' : [10,15,16],
							'type' : key,
					...
					*/
					...
					html += '<div class="epie-export-progress__row epie-export-progress__row--';
					
					if ( epie_export_data.data[key].ids.length == 0 ) {
						html += 'finished';
					} else if ( epie_export_data.data[key].active ) {
						html += 'in-progress';
					} else {
						html += 'wait';
					}
					...
					let text = epie_export_data.data[key].text;
					text = text.replace('$count', (epie_export_data.data[key].total - epie_export_data.data[key].ids.length) + '/' + epie_export_data.data[key].total );
					
					html += `<div class="epie-export__title">${text}</div>`;

					if ( epie_export_data.data[key].ids.length == 0 ) {
						html += `<div class="epie-export__icon"><i class="epkbfa epkbfa-check-circle"></i></div>`;
					} else if ( epie_export_data.data[key].active ) {
						html += `<div class="epie-export__icon"><i class="epkbfa epkbfa-spinner"></i></div>`;
					} else {
						html += `<div class="epie-export__icon"><i class="epkbfa epkbfa-pause-circle"></i></div>`;
					}
					
					html += `</div>`;
					
					// progress bar 
					total_count += epie_export_data.data[key].total;
					current_count += (epie_export_data.data[key].total - epie_export_data.data[key].ids.length);
				}
			} else if (! epie_export_data.info.error ) {
				// first start
				let title = $('#epie-content-export .epie-progress').data('start');
			...
			// progress bar 
			if (total_count) {
				$('#epie-content-export .epie-progress__bar div').css({
					'width' : (100*current_count/total_count) + '%'
				});
			} else {
				$('#epie-content-export .epie-progress__bar div').css({
					'width' : '0%'
			...
			if ( epie_export_data.info.file ) {
				$('#export_file_link').attr( 'href', epie_export_data.info.file );
				$('#export_file_link').show();
			} else {
				$('#export_file_link').hide();
			}
		...
		$('#epie-content-export .epie-progress__log').html(html);
		
	}
	
	// first step - get all things for export and fill them to the storage.	
	function epie_get_export_data() {
		let postData = {
			_wpnonce_kb_import_export: $('#_wpnonce_kb_import_export').val(),
			action: 'epie_get_export_data',
			form: $('#export-main-form').serialize()
		};
		...
		$.ajax({
			...
			url: ajaxurl,
			beforeSend: function (xhr) {
				// clear data storage and update progress view
				epie_export_data.data = {};
				epie_export_data.info.success = '';
				epie_export_data.info.error = '';
				epie_export_data.info.file = '';
				epie_export_data.user_data.file_id = '';
				epie_export_data.info.in_progress = true;
				epie_export_data.user_data = {};
				epie_update_export_progress_view();
				$('#epie-content-export .epie-progress').show();
			}
		}).done(function (response) {
			...
			if (response.error) {
				...
				epie_export_data.data = response.data;
				epie_export_data.user_data.tag = response.tag;
				epie_export_data.user_data.description = response.description;
				epie_export_data.user_data.file_id = response.file_id;
				epie_run_export();
			}
		});
	}
	
	// recursive function to run export step by step 
	function epie_run_export() {
		if ( Object.keys(epie_export_data.data).length && !epie_export_data.info.error ) {
			// make all data-types not-active 
			for ( let key in epie_export_data.data ) {
				epie_export_data.data[key].active = false;
			}
			...
			// find next step 
			for ( let key in epie_export_data.data ) {
				if ( epie_export_data.data[key].ids.length ) {
					// set status 
					epie_export_data.data[key].active = true;
					ids = epie_export_data.data[key]['ids'].slice(-epie_export_data.info.items_per_request);
					type = key;
					break;
				}
			}
			...
			let postData = {
				...
				type: type,
				ids: ids,
				file_id: epie_export_data.user_data.file_id,
				epie_kb_id: $('input[name=epie_kb_id]').val(),
				description: epie_export_data.user_data.description,
				tag: epie_export_data.user_data.tag
			};
			...
			$.ajax({
				...
				// clear exported ids
				if ( ( 'finish' !== type ) && ids && ( response.status == 'success' ) ) {
					epie_export_data.data[type]['ids'].splice(-epie_export_data.info.items_per_request, epie_export_data.info.items_per_request);
				}
				...
				if ( ids.length ) {
					epie_run_export(); // run if we have more steps 
				}
				
			}).fail( function(response, textStatus, error) {
				if ( epie_export_data.info.items_per_request > 1 ) {
					// if server can't handle a lot of results 
					epie_export_data.info.items_per_request--;
					epie_run_export();
				} else {
					// if something else 
					let errorMessage = ( error ? ' [' + error + ']' : 'unknown error' );
					if ( errorMessage !== '' ) {
						bottomNoticeMessage( errorMessage, 'error' );
						return;
					}
				}
			});
		...
		// recursion exit 
		epie_export_data.info.in_progress = false;
		epie_update_export_progress_view();
		return;
	}

	function epie_export_article_pack() {
		if (epie_article_ids.length) {
			let current_pack = epie_article_ids[epie_article_ids.length - 1];
			...
			$.ajax({
				...
				if (response.status == 'success') {
					epie_article_ids.pop();
					$('#epie-content-export .epie-export-log').append( `Done, ${epie_article_ids.length} left <br>`);
				}
				
				epie_export_article_pack();
			});
		} else {
			// get json 
			let postData = {
			...
			$.ajax({
				...
				$('.epie-export-json').append( `<a href="${response.link}" target="_blank">Get JSON</a><br>` );
				//$('.epie-export-json').append( JSON.stringify(response.data) );
			});
			...
			$.ajax({
				...
				$('.epie-export-json').append( `<a href="${response.link}" target="_blank">Get CSV</a>` );
			});
		}
	}
	
	function epie_get_article_ids() {
		$('#epie-content-export .epie-export-log').append('Get Article list <br>');
		
		let postData = {
			action: 'epie_kb_get_article_ids'
		};
		...
		$.ajax({
			...
			$('#epie-content-export .epie-export-log').append('Done <br>');
			if (response.data) {
				epie_article_ids = response.data;
	...
	$('#epie-start-export-kb-btn').click(function(){
		epie_get_article_ids();
	});
	...
	$( '#export-main-form' ).submit( function() {
		$( '.epie-dialog-box-form' ).toggleClass( 'epie-dialog-box-form--active' );
		return false;
	});
	
	// Close Dialog box
	$( '.epie-dbf__footer__cancel__btn, .epie-dbf__close' ).on('click', function(){
		$( this ).parents( '.epie-dialog-box-form' ).removeClass( 'epie-dialog-box-form--active' );
	});
	
	// run export 
	$( '#epie-export-form #epie-accept-button' ).on('click', function(){
		// Close Dialog
		$( this ).parents( '.epie-dialog-box-form' ).removeClass( 'epie-dialog-box-form--active' );
		...
		// Get data (next step find inside this function)
		epie_get_export_data();
	});

...
class EPIE_Logging {
	...
	const LOGGING_OPTION_NAME =  'epie_error_log';
	...
	public static function add_log( $error_message, $param1='', $param2=null ) {
		...
		$variable = EPIE_Utilities::get_variable_string( $variable );
		...
		// prepare error message
		$error_message = EPIE_Utilities::substr( $error_message, 0, 3000);
		...
		$kb_id = defined('EM'.'KB_PLUGIN_NAME') ? $kb_id : EPIE_KB_Config_DB::DEFAULT_KB_ID;

		// add new error log entry but remove oldest one if more than max
		// FUTURE TODO log current user
		$error_log[] = array( 'plugin' => EPIE_PLUGIN_NAME, 'kb' => $kb_id, 'date' => date("Y-m-d H:i:s"), 'message' => $error_message, 'trace' => $stack_trace );
		...
		// save the error log
		EPIE_Utilities::save_wp_option( self::LOGGING_OPTION_NAME, $error_log, true );
	}
	...
	public static function get_logs() {
		$logs = EPIE_Utilities::get_wp_option( self::LOGGING_OPTION_NAME, array(), true );
	...
	private static function can_log_message() {
		...
		$is_debug_on = EPIE_Utilities::get_wp_option( EPIE_KB_Core::EPIE_KB_DEBUG, false );
	...
	public static function generateStackTrace()	{
		...
		foreach( debug_backtrace() as $trace ) {
			...
			$file = EPIE_Utilities::substr($file, 1, 500);
		...
		$stackMsg = EPIE_Utilities::substr( $stackMsg, 0, 2000);

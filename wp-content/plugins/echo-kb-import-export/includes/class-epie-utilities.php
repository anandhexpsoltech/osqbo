...
class EPIE_Utilities {
	...
	public static function get_kb_post_secure( $post_id, $only_published=true ) {
		...
		if ( ! EPIE_KB_Handler::is_kb_post_type( $post->post_type ) ) {
			return null;
		}
	...
	public static function get_bottom_notice_message_box($message, $title='', $type='success' ) {
		...
		if ( ! empty($EZSQL_ERROR) ) {
			EPIE_Logging::add_log( 'Database error', $EZSQL_ERROR );
		...
		return
			"<div class='eckb-bottom-notice-message'>
				...
				<div class='epie-close-notice epkbfa epkbfa-window-close'></div>
			</div>";
	}
	...
	public static function dialog_box_form( $values ) { ?>
		...
		<div id="<?php echo $values[ 'id' ]; ?>" class="epie-dialog-box-form">

			<!---- Header ---->
			<div class="epie-dbf__header">
			...
			<form class="epkb-dbf__form">				<?php
				if ( isset($values['form_inputs']) ) {
					foreach ( $values['form_inputs'] as $input ) {
						echo '<div class="epie-dbf__form__input">' . $input . '</div>';
					}
				}; ?>
			</form>

			<!---- Footer ---->
			<div class="epie-dbf__footer">

				<div class="epie-dbf__footer__accept <?php echo isset($values['accept_type']) ? 'epie-dbf__footer__accept--'.$values['accept_type'] : 'epie-dbf__footer__accept--success'; ?>">
					<span id="epie-accept-button" class="epie-dbf__footer__accept__btn">
						<?php echo $values['accept_label'] ? $values['accept_label'] : __( 'Accept', 'echo-knowledge-base' ); ?>
					</span>
				</div>

				<div class="epie-dbf__footer__cancel">
					<span class="epkb-dbf__footer__cancel__btn"><?php _e( 'Cancel', 'echo-knowledge-base' ); ?></span>
				</div>

			</div>

			<div class="epie-dbf__close epkbfa epkbfa-times"></div>
	...
	public static function sanitize_get_id( $id ) {
		...
		if ( ! self::is_positive_int( $id ) ) {
			EPIE_Logging::add_log( 'Error occurred (04)', $id );
	...
	public static function retrieve_and_sanitize_form( $form, $all_fields_specs ) {
		...
		foreach( $submitted_fields as $submitted_key => $submitted_value ) {
			...
			if ( ! empty($all_fields_specs[$submitted_key]['type']) && $all_fields_specs[$submitted_key]['type'] == EPIE_Input_Filter::WP_EDITOR ) {
				$sanitized_fields[$submitted_key] = wp_kses_post( $submitted_value );
			} elseif ( ! empty( $all_fields_specs[$submitted_key]['type'] ) && ! empty( $all_fields_specs[$submitted_key]['allowed_tags'] ) && $all_fields_specs[$submitted_key]['type'] == EPIE_Input_Filter::TEXT ) {
	...
	public static function get_wp_option( $option_name, $default, $is_array=false, $return_error=false ) {
		...
		if ( $return_error && $option === null && ! empty($wpdb->last_error) ) {
			EPIE_Logging::add_log( "DB failure: " . $wpdb->last_error, 'Option Name: ' . $option_name );
	...
	public static function save_wp_option( $option_name, $option_value, $sanitized ) {
		...
		if ( $result === false ) {
			EPIE_Logging::add_log( 'Failed to update option', $option_name );
	...
	public static function get_postmeta( $post_id, $meta_key, $default, $is_array=false, $return_error=false ) {
		...
		if ( $return_error && $option === null && ! empty($wpdb->last_error) ) {
			EPIE_Logging::add_log( "DB failure: " . $wpdb->last_error, 'Meta Key: ' . $meta_key );
			return new WP_Error(__( 'Database failure', 'echo-knowledge-base' ), $wpdb->last_error);
		}
	...
	public static function save_postmeta( $post_id, $meta_key, $meta_value, $sanitized ) {
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			EPIE_Logging::add_log( "DB failure: " . $wpdb->last_error );
			return new WP_Error(__( 'Database failure', 'echo-knowledge-base' ), $wpdb->last_error);
		}
		...
		if ( empty($result) ) {
			if ( false === $wpdb->query( $wpdb->prepare( "INSERT INTO $wpdb->postmeta (`meta_key`, `meta_value`, `post_id`) VALUES (%s, %s, %d)", $meta_key, $serialized_value, $post_id ) ) ) {
				EPIE_Logging::add_log("Failed to insert meta data. ", $meta_key);
			...
			if ( false === $wpdb->query( $wpdb->prepare( "UPDATE $wpdb->postmeta SET meta_value = %s WHERE meta_key = '%s' AND post_id = %d", $serialized_value, $meta_key, $post_id ) ) ) {
				EPIE_Logging::add_log("Failed to update meta data. ", $meta_key);
				return new WP_Error( '33', __( 'Failed to update meta data', 'echo-knowledge-base' ) );
			}
		}

		if ( $result === false ) {
			EPIE_Logging::add_log( 'Failed to update meta key', $meta_key );
	...
	public static function delete_postmeta( $post_id, $meta_key ) {
		...
		if ( false === $wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->postmeta WHERE post_id = %d and meta_key = '%s'", $post_id, $meta_key ) ) ) {
			EPIE_Logging::add_log("Could not delete post '" . self::get_variable_string($meta_key) . "'' metadata: ", $post_id);
			return false;
		}
    ...
    public static function get_kb_categories_unfiltered( $kb_id, $order_by='name' ) {
        ...
        $kb_category_taxonomy_name = EPIE_KB_Handler::get_category_taxonomy_name( $kb_id );
	...
	public static function get_article_categories_unfiltered( $kb_id, $article_id ) {
		...
		$post_taxonomy_objs = $wpdb->get_results( $wpdb->prepare(
											...
											EPIE_KB_Handler::get_category_taxonomy_name( $kb_id ), $article_id ) );
    ...
    public static function get_kb_category_unfiltered( $kb_id, $kb_category_id ) {
	    $term = get_term_by('id', $kb_category_id, EPIE_KB_Handler::get_category_taxonomy_name( $kb_id ) );
	    if ( empty($term) || ! $term instanceof WP_Term ) {
		    EPIE_Logging::add_log("Category is not KB Category: " . $kb_category_id . "(35)", $kb_id);
		    return false;
	    }
	...
	public static function get_kb_category_by_slug_unfiltered( $kb_id, $kb_category_slug ) {
		$term = get_term_by('slug', $kb_category_slug, EPIE_KB_Handler::get_category_taxonomy_name( $kb_id ) );
		if ( empty($term) || ! $term instanceof WP_Term ) {
			EPIE_Logging::add_log("Category is not KB Category: " . $kb_category_slug . "(34)", $kb_id);
			return false;
		}
    ...
    public static function insert_kb_category( $kb_id, $category_name, $args=array() ) {
	    ...
	    if ( is_wp_error($new_term) ) {
		    EPIE_Logging::add_log( 'Failed to insert category. cat name: ' . $category_name . ', taxonomy: ' . $kb_category_taxonomy_name . ' kb id: ' . $kb_id, $args, $new_term );
		    return null;
	    }
	    if ( ! isset($new_term['term_id']) ) {
		    EPIE_Logging::add_log( 'Failed to insert category. cat name: ' . $category_name . ', taxonomy: ' . $kb_category_taxonomy_name . ' kb id: ' . $kb_id, $args );
		    return null;
	    }
	...
	public static function get_kb_id( $post=null ) {
		...
		if ( ! empty($post) && $post instanceof WP_Post ) {
			$kb_id = EPIE_KB_Handler::get_kb_id_from_post_type( $post->post_type );
		}

		$kb_id = empty($kb_id) || is_wp_error($kb_id) ? ( empty($eckb_kb_id) ? '' : $eckb_kb_id ) : $kb_id;
		if ( empty($kb_id) ) {
			EPIE_Logging::add_log("KB ID not found", $kb_id);
			return null;
		}

 ...
 */
class EPIE_Add_Ons_Page {

	public function __construct() {
		add_filter( EPIE_KB_Core::EPIE_KB_LICENSE_FIELD, array( $this, 'display_license_status_update') );
		add_action( 'wp_ajax_epie_handle_license_request', array( $this, 'handle_license_request' ) );
		add_action( 'wp_ajax_nopriv_epie_handle_license_request', array( $this, 'user_not_logged_in' ) );
	}
	...
	public function display_license_status_update( $output ) {
		...
		$info = __( 'Enter your license key for the ', 'echo-knowledge-base' ) . EPIE_License_Handler::PLUGIN_NAME . __( ' add-on.', 'echo-knowledge-base' );
		$output .= '<li class="ekb-add-on">';
		$output .= '<div class="ekb-add-on-title">'
					.'<h2>'. EPIE_License_Handler::PLUGIN_NAME . '</h2>' .
			            '<div class="epie_license_info">' .
		                   '<span class="ep_font_icon_info"></span><p class="ep_font_icon_info_content hidden">' . esc_html($info) . '</p>' .
		                '</div>'.
			       '</div>'.
		           '<div class="ekb-license-check" id="epie_license_check">' . __( 'Updating license status. Please wait.', 'echo-knowledge-base' ) . '</div>' .
		           '<div id="epie-ajax-in-progress" style="display:none;">' . esc_html__( 'Updating license...', 'echo-knowledge-base' ) . '</div>';
		$output .= '</li>';
	...
	public function handle_license_request() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			EPIE_Utilities::ajax_show_error_die( __( 'You do not have permission to view license info.', 'echo-knowledge-base' ) );
		}

		$epie_instance = epie_get_instance();
		if ( empty($epie_instance) || !$epie_instance instanceof Echo_KB_Import_Export ) {
			EPIE_Utilities::ajax_show_error_die( __( 'Failed to initialize (M99).', 'echo-knowledge-base' ) );
		}

		$command = EPIE_Utilities::post('command');
		$command = ! empty($command) && in_array( $command, array( 'get_license_info', 'save' ) ) ? $command : '';
		if ( empty($command) ) {
			EPIE_Utilities::ajax_show_error_die( __( 'Unknown licensing command: ', 'echo-knowledge-base' ) . $command );
		}

		if ( empty( $epie_instance->license_handler ) ) {
			EPIE_Utilities::ajax_show_error_die(  __( 'Failed to initialize (L99)', 'echo-knowledge-base' ) );
		}

		$license_handler = $epie_instance->license_handler;
	 ...
	 *
	 * @param EPIE_License_Handler $license_handler
	 * @param $license_data
	...
	private function get_license_state( $license_handler, $license_data ) {
		...
		if ( ! empty($license_error) && $license_handler->is_connection_error( $license_error ) ) {
			EPIE_Logging::add_log("Could not contact licensing server.", ( isset($license_data->license) ? $license_data->license : '' ) );
			$cached_info = $license_handler->get_cached_info();
		...
		EPIE_Utilities::save_wp_option('epie_last_license_check', array(), true);
	...
	private function license_fields( $status, $status_msg, $expiry_date, $latest_add_on_version, $license_key ) {
		...
		<div class="ekb-license-key">
			<ul>
				...
				<li><input type="text" name="<?php echo EPIE_License_Handler::LICENSE_KEY_OPTION_ID; ?>" id="<?php echo EPIE_License_Handler::LICENSE_KEY_OPTION_ID; ?>"
				...
				<li><span class="epie_license_action"><input id="epie_save_btn" class="primary-btn" type="button" name="epie_save_btn" value="<?php echo  __( 'Save', 'echo-knowledge-base' ); ?>"/>
				<input type="hidden" id="_wpnonce_epie_license_key" name="_wpnonce_epie_license_key" value="<?php echo wp_create_nonce( "_wpnonce_epie_license_key" ); ?>"/></span></li>
			</ul>
		</div>
		...
		<div class="ekb-license-status">
			<ul>
				...
				<li><span id="epie_license_key_status"><?php echo wp_kses_post( $status_msg ); ?></span></li>
			</ul>
		</div>
		...
		<div class="ekb-license-expiry">			<?php
			...
			$expiry_msg = empty($expiry_msg) ? EPIE_Utilities::get_formatted_datetime_string( $expiry_date ) : $expiry_msg;			?>
		...
		<div class="ekb-license-version">			<?php
			...
			$new_version_available = version_compare( Echo_KB_Import_Export::$version, $latest_add_on_version, "<" );
			...
			<ul>
				...
				<li><?php _e( 'Installed:', 'echo-knowledge-base' ); ?> <?php echo Echo_KB_Import_Export::$version; ?></li>
				...
				<li>					<?php
					...
					if ( $new_version_available && $status == 'valid' ) {
						$name = plugin_basename( Echo_KB_Import_Export::$plugin_file );
	...
	public function user_not_logged_in() {
		EPIE_Utilities::ajax_show_error_die( '<p>' . __( 'You are not logged in. Refresh your page and log in', 'echo-knowledge-base' ) . '.</p>', __( 'Cannot save your changes', 'echo-knowledge-base' ) );
	}
}
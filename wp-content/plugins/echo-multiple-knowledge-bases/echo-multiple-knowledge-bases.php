...
final class Echo_Multiple_Knowledge_Bases {
	...
	private function setup_backend_classes() {
		...
		if ( $pagenow == 'post.php' ) {
			...
			$post = EMKB_Utilities::get_kb_post_secure( $post_id, false );
...
function emkb_display_version_error() {
	...
	if ( $plugins_check == 'core_missing' ) {
		echo '<div class="error"><p>KB - Multiple Knowledge Bases requires <a href="https://wordpress.org/plugins/echo-knowledge-base/"
	...
	if ( $plugins_check == 'core_too_old' ) {
		echo '<div class="error">Multiple Knowledge Bases plugin requires at least ' . Echo_Multiple_Knowledge_Bases::$needs_min_core_version .
		...
		echo '<div class="error">' . $kb_plugin_name . ' plugin requires the most recent version of the Multiple Knowledge Bases plugin. The old version of Multiple Knowledge Bases stays
			<strong>disabled</strong> until updated. <a href="' . $add_on_url .'" target="_blank">Check its status here.</a></p></div>';
	} else {
		// should not happen
		echo '<div class="error">Multiple Knowledge Bases: unknown add-on version conflict.</div>';
	}
}

...
class ASEA_KB_Config_DB {
	...
	function get_kb_configs( $skip_check=false ) {
		...
		foreach ( $kb_options as $ix => $row ) {
			...
			if ( count($config) < 100 ) {
				ASEA_Logging::add_log("Found KB configuration is incomplete", count($config));
			}
			...
			if ( ASEA_Utilities::is_wpml_enabled_addon( $kb_id ) ) {
	...
	public function get_kb_config( $kb_id, $default_on_error=true ) {
		...
		if ( empty($config) || ! is_array($config) ) {
			...
			return ( $default_on_error ? ASEA_KB_Config_Specs::get_default_kb_config() :
				new WP_Error('get_kb_config', 'Did not find KB Config. Try to deactivate and reactivate KB plugin to see if this fixes the issue. ' . ASEA_Utilities::contact_us_for_support()) );
		}

		if ( count($config) < 50 ) {
			ASEA_Logging::add_log("Found ASEA configuration is incomplete", count($config));
		}
	...
	public function get_value( $kb_id, $setting_name, $default = '' ) {
		...
		if ( is_wp_error( $kb_config ) ) {
			ASEA_Logging::add_log( "Could not retrieve KB configuration (15). Settings name: ", $setting_name, $kb_config );
			return $default;
		}
	...
	public function update_kb_configuration( $kb_id, array $config, $upsert=true ) {
		...
		if ( is_wp_error($sanitized_config) ) {
			ASEA_Logging::add_log( 'Could not update ASEA configuration', $kb_id, $sanitized_config );
	...
	private function save_kb_config( array $config, $kb_id, $upsert=true ) {
		...
		if ( $result === false ) {
			ASEA_Logging::add_log( 'Failed to update kb config for kb_id', $kb_id );

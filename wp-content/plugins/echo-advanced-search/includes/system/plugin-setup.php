		...
		if ( class_exists('ASEA_KB_Config_DB') ) {
			...
			foreach ( $all_kb_ids as $kb_id ) {
				...
				if ( is_wp_error( $add_on_config ) ) {
					...
					// if possible port search configuration from KB Core or EL AY
					$core_kb_config = ASEA_KB_Core::get_kb_config( $kb_id );
					if ( ! is_wp_error($core_kb_config) ) {
						$add_on_defaults = ASEA_Search_Utilities::set_search_config_from_current_config( $kb_id, $core_kb_config, $add_on_defaults );
					}

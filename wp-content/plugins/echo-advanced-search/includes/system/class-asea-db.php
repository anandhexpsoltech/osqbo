...
abstract class ASEA_DB {
	...
	public function get_by_primary_key( $primary_key_value ) {
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			ASEA_Logging::add_log( "DB failure: " . $wpdb->last_error, 'Primary Key: ' . $primary_key_value );
	...
	public function get_a_row_by_column_value( $column, $column_value ) {
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			ASEA_Logging::add_log( "DB failure: " . $wpdb->last_error, 'Column - value: ' . $column . ' - ' . $column_value );
			return new WP_Error('DB failure', $wpdb->last_error);
		}
	...
	public function get_column_value_by_primary_key( $column, $primary_key ) {
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			ASEA_Logging::add_log( "DB failure: " . $wpdb->last_error, 'Column: ' . $column. ', Primary Key: ' . $primary_key );
			return new WP_Error('DB failure', $wpdb->last_error);
		}
	...
	public function get_column_value_by( $select_column, $column_name, $column_value ) {
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			ASEA_Logging::add_log( "DB failure: " . $wpdb->last_error, 'Select Column: ' . $select_column. ', Column - value: ' . $column_name . ' - ' . $column_value );
			return new WP_Error('DB failure', $wpdb->last_error);
		}
	...
	public function get_count_rows_range( $kb_id, $date_column_name, $date_from, $date_to, $where_condition='' ) {
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			ASEA_Logging::add_log( "DB failure: " . $wpdb->last_error, $date_from . ' - ' . $date_to );
			return new WP_Error('DB failure', $wpdb->last_error);
		}

...
class ASEA_Search_DB extends ASEA_DB  {
	 ...
	 *                    - NULL on failure
	 */
	public function get_most_popular_searches( $kb_id, $date_from, $date_to, $limit ) {
		...
		$result = $this->get_rows_by_date_range( $kb_id, 'search_date', $date_from, $date_to, 'times DESC', 'search_keywords', $limit, ' AND count > 0 ' );
	 ...
	 *                    - NULL on failure
	 */
	public function get_no_results_searches( $kb_id, $date_from, $date_to, $limit ) {
	 ...
	 * @return int
	 */
	public function get_search_count( $kb_id, $date_from, $date_to ) {
		...
		$result = $this->get_count_rows_range( $kb_id, 'search_date', $date_from, $date_to );
		if ( is_wp_error( $result) ) {
	 ...
	 * @return int
	 */
	public function get_no_result_search_count( $kb_id, $date_from, $date_to ) {
		...
		$result = $this->get_count_rows_range( $kb_id, 'search_date', $date_from, $date_to, ' AND count = 0 ' );
		if ( is_wp_error( $result) ) {
			ASEA_Logging::add_log("Cannot get Search count: ", $kb_id);
			return 0;
		}
	 ...
	 */
    public function insert_search_record( $kb_id, $user_id, $user_uid, $user_input, $search_keywords, $count ) {
        ...
        // insert the record
	    $record = array('kb_id' => $kb_id, 'user_id' => $user_id, 'user_uid' => $user_uid, 'user_input' => $user_input, 'search_keywords' => $search_keywords, 'count' => $count);

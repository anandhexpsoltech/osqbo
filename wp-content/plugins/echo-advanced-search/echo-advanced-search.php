...
final class Echo_Advanced_Search {
	...
	private function handle_action_request( $action ) {
		...
		if ( ! ASEA_KB_Handler::is_kb_request() ) {
			return;
		}
	}
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( in_array($action,
			array(ASEA_KB_Core::ASEA_CHANGE_ARTICLE_PAGE_CONFIG_AJAX, ASEA_KB_Core::ASEA_CHANGE_MAIN_PAGE_CONFIG_AJAX,
	...
	private function setup_backend_classes() {
		...
		if ( $pagenow == 'post.php' ) {
			...
			$post = ASEA_Utilities::get_kb_post_secure( $post_id, false );
		...
		if ( ! empty($is_kb_request) || $pagenow == 'post.php' || $pagenow == 'post-new.php' ) {
			add_action( 'admin_enqueue_scripts', 'asea_load_admin_plugin_pages_resources' );
		}
...
function asea_display_version_error() {
	...
	if ( $plugins_check == 'core_missing' ) {
		echo '<div class="error"><p>KB - Advanced Search requires <a href="https://wordpress.org/plugins/echo-knowledge-base/"
	...
	if ( $plugins_check == 'core_too_old' ) {
		echo '<div class="error">Advanced Search plugin requires at least ' . Echo_Advanced_Search::$needs_min_core_version .
		...
		echo '<div class="error">' . $kb_plugin_name . ' plugin requires the most recent version of the Advanced Search plugin. The old version of Advanced Search stays
			<strong>disabled</strong> until updated. <a href="' . $add_on_url .'" target="_blank">Check its status here.</a></p></div>';
	} else {
		// should not happen
	    echo '<div class="error">Advanced Search: unknown add-on version conflict.</div>';
	}
}

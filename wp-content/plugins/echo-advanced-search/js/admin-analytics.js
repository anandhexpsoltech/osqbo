jQuery(document).ready(function($) {
	...
	display_asea_pie_chart( 'asea-popular-searches-data' );
	display_asea_pie_chart( 'asea-no-result-popular-searches-data' );
	...
	if ( $( '.asea-reset-container' ).length > 0 ){
		...
		$( '.asea-reset-analytics' ).click( function(){
		...
		$('#asea-reset-search-data #asea-accept-button').click(reset_analytics);

		$('#asea-reset-cancel').click(function(){
		...
		function reset_analytics() {
			...
			$.ajax({
				...
				$('#asea-ajax-in-progress').dialog('close');
				$('.asea-dbf__footer__cancel__btn').click();
